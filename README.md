...
### `models.json` - 模型列表
这个文件包含了 LMArena 平台上的模型名称到其内部ID的映射。
*   **重要**: 现在，程序**不会**自动更新此文件。你需要手动维护这个列表。
*   当客户端请求一个模型时，`api_server.py` 会在此文件中查找对应的模型ID，并将其包含在发往 LMArena 的请求中。
...
这是**最重要**的一步。你需要获取一个有效的会话 ID 和消息 ID，以便程序能够正确地与 LMArena API 通信。此步骤用于更新 `config.jsonc` 中的全局默认ID，或为 `model_endpoint_map.json` 提供新的会话ID。
